...
chrome.webNavigation.onCompleted.addListener(function checkForValidUrl2(tabId) {
    ...
    // setup the valid tab, get the data and display the icon
    updateAttendeeInfo(tabId.tabId);
}, {url: [{urlContains: 'ce.app.neoncrm.com/np/admin/event/contactSelect.do'}] });

chrome.webNavigation.onCompleted.addListener(function checkForValidUrl2(tabId) {
    console.log("The appropriate page has finished loading, moving on");
    // setup the valid tab, get the data and display the icon
    updateAttendeeInfo(tabId.tabId);
}, {url: [{urlContains: 'trial.ce.app.neoncrm.com/np/admin/event/contactSelect.do'}] });

chrome.webNavigation.onCompleted.addListener(function checkForValidUrl2(tabId) {
    console.log("The appropriate page has finished loading, moving on");
    // setup the valid tab, get the data and display the icon
    updateRegistrationsInfo(tabId.tabId);
}, {url: [{urlContains: 'ce.app.neoncrm.com/np/admin/event/eventRegDetails.do'}] });

chrome.webNavigation.onCompleted.addListener(function checkForValidUrl2(tabId) {
    console.log("The appropriate page has finished loading, moving on");
    // setup the valid tab, get the data and display the icon
    updateRegistrationsInfo(tabId.tabId);
}, {url: [{urlContains: 'trial.ce.app.neoncrm.com/np/admin/event/eventRegDetails.do'}] });
...
chrome.omnibox.onInputChanged.addListener(
    function(text, suggest) {
        ...
        if (text.match(/^[a,A,k,K,c,C,t,T][1-9]+/)) {
            ...
            suggest([
                {content: "https://ce.app.neoncrm.com/np/admin/event/attendeeEdit.do?id="+badgeNumId, description: "Navigate to this Badge Number"},
                {content: "https://ce.app.neoncrm.com/np/admin/searchResult.do?key="+text+"&search=Search&searchType=1", description: "Search For This Account Keyword"},
                {content: "https://ce.app.neoncrm.com/np/admin/event/registrationSearch.do?query.eventId=202", description: "Open a general search for registrants/attendees"}
            ]);
        } else {
            suggest([
                {content: "https://ce.app.neoncrm.com/np/admin/searchResult.do?key="+text+"&search=Search&searchType=1", description: "Search For This Account Keyword"},
                {content: "https://ce.app.neoncrm.com/np/admin/event/attendeeEdit.do?id="+text, description: "Navigate to this Badge Number"},
                {content: "https://ce.app.neoncrm.com/np/admin/event/registrationSearch.do?query.eventId=232", description: "Open a general search for registrants/attendees"}
            ]);
        }
    });
        ...
        chrome.tabs.getSelected(null, function(tab)
        {
            ...
            if (text.substr(0, 8) == 'https://') {
                ...
                if (text.match(/^[a,A,k,K,c,C,t,T][1-9]+/)) {
                    ...
                    url = "https://ce.app.neoncrm.com/np/admin/event/attendeeEdit.do?id="+badgeNumId;
                } else {
                    if (text.trim()=="") {
                        console.log("User did not enter anything, so we will take them to a generic search");
                        url = "https://ce.app.neoncrm.com/np/admin/event/registrationSearch.do?query.eventId=202";
                    } else {
                        console.log("It doesn't look like a badge number so lets just search neon accounts. User entered: "+text);
                        url = "https://ce.app.neoncrm.com/np/admin/searchResult.do?key="+text+"&search=Search&searchType=1";
                    }
                }
            }
